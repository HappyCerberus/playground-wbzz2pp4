...
1. if we are increasing or decreasing a key and there is no other key at both the old and the new value, we don't change the keys position in the sorted sequence
2. if we are increasing or decreasing a key and there was another key at the old value but no other key at the new value, we will need to insert a new element into the sequence one position away from the keys previous location
3. if we are increasing or decreasing a key and there was no other key at the old location, but there are keys at the new value, we will need to remove an element from the sequence, from the keys previous location
